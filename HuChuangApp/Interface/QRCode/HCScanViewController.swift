...
extension HCScanViewController: AVCaptureMetadataOutputObjectsDelegate {
    ...
    func scanResult(_ result: String) {
        ...
        if let _ = Int(result) {
            ...
            NoticesCenter.alert(title: "提示", message: "非核销码", okTitle: "重新扫码", callBackOK:  { [weak self] in
                if self?.canUseSystemCamera() == true && self?.qRScanView.superview != nil {
                    self?.zxingObj.start()
                }
            })
        }
